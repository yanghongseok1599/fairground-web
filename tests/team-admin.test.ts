...
const players = [
  { id: "captain-1", role: "captain", isApproved: true, createdAt: 30 },
...
assert.equal(canManageTeamAsDirector({ id: "captain-1", role: "captain", teamId: "team-123" }, { id: "team-123", captainId: "someone-else" }), true);
assert.equal(canManageTeamAsDirector({ id: "owner-1", role: "player", teamId: "team-123" }, { id: "team-123", captainId: "owner-1" }), true);
assert.equal(canManageTeamAsDirector({ id: "admin-1", role: "admin", teamId: "other" }, { id: "team-123" }), true);
assert.equal(canManageTeamAsDirector({ id: "player-1", role: "player", teamId: "team-123" }, { id: "team-123" }), false);
assert.equal(canManageTeamAsDirector(null, { id: "team-123" }), false);

const buckets = getTeamAdminMemberBuckets(players);
assert.deepEqual(buckets.approved.map((player) => player.id), ["captain-1", "player-1"]);
