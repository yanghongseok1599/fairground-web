...
import { PracticeSpectatorView } from "./spectator-view";
import { readRoomFrame, roleLabel, roomLink, roomStorageKey, type RoomRole } from "./protocol";
...
export function SharedRoomPage({ room, role }: { room: string; role?: RoomRole }) {
  if (!role) return <section className="mx-auto max-w-2xl space-y-5 px-4 py-10">
    ...
    <div className="flex flex-wrap gap-3"><Button onClick={() => location.assign(roomLink(location.origin, room, "referee"))}>심판으로 입장</Button><Button variant="outline" onClick={() => location.assign(roomLink(location.origin, room, "admin"))}>관리자로 입장</Button><Button variant="outline" onClick={() => location.assign(roomLink(location.origin, room, "spectator"))}>참가자 중계 보기</Button></div>
...
function ConnectedRoom({ room, role }: { room: string; role: RoomRole }) {
  ...
  const duplicate = role !== "spectator" && activeSeat !== null && activeSeat !== identity.id;
  ...
  const copy = async (target: RoomRole) => {
    try { await navigator.clipboard.writeText(roomLink(location.origin, room, target)); setCopied(`${roleLabel(target)} 링크를 복사했습니다.`); }
  ...
  return <>
    <section className="border-b border-blue-200 bg-blue-50 px-4 py-5 text-slate-900">
      <div className="mx-auto max-w-3xl space-y-3">
        <a href="/match-simulation" className="text-sm underline">공유 경기 목록으로</a>
        <div className="flex items-center gap-2 font-bold text-blue-700"><Users className="h-4 w-4" />함께 테스트 · {roleLabel(role)}</div>
        <h1 className="text-2xl font-black">공유 테스트 경기 · {room.slice(0, 8)}</h1>
        <p className="text-sm">심판과 운영 관리자는 전체 명단에서 기록하고, 참가자는 경기장 중계로 함께 봅니다.</p>
        ...
        {role !== "spectator" && <div className="flex flex-wrap gap-2">{(["referee", "admin", "spectator"] as const).map(target => <Button key={target} variant="outline" onClick={() => copy(target)}><Copy className="mr-2 h-4 w-4" />{roleLabel(target)} 링크 복사</Button>)}</div>}
        {role !== "spectator" && <div className="flex flex-wrap gap-4 text-sm underline"><a href={roomLink(location.origin, room, "referee")}>심판 입장 링크</a><a href={roomLink(location.origin, room, "admin")}>관리자 입장 링크</a><a href={roomLink(location.origin, room, "spectator")}>참가자 중계 보기</a></div>}
        ...
        {observing && <p role="status" className="text-sm text-blue-800">다른 관리자와 같은 경기를 보고 있습니다. 기록 입력과 초기화는 먼저 접속한 관리자가 진행합니다.</p>}
    ...
    {role === "spectator" ? <PracticeSpectatorView session={session} /> : role === "admin" ? <PracticeAdminView session={session} active={active} observing={observing} /> :
