...
import type { createPracticeStore } from "./store";

type PracticeApi = ReturnType<typeof createPracticeStore>;

export function SimulationWorkspace({ store, storageNotice = "", shared = false }: { store: PracticeApi; storageNotice?: string; shared?: boolean }) {
    ...
    <MatchControlStoreContext.Provider value={store}>
      <section className="border-b border-blue-200 bg-blue-50 px-4 py-5 text-slate-900">
        <div className="mx-auto max-w-3xl space-y-3">
          ...
          <p className="text-xs leading-relaxed text-slate-600">60배속이면 12초에 경기 시간이 끝납니다. 마지막에는 ‘경기 종료’에서 MOM을 선택하세요. {shared ? "관리자 화면에 점수와 기록이 함께 표시됩니다." : "기록은 이 탭에만 보관되며 새로고침하면 일시정지 상태로 이어집니다."}</p>
      ...
      <Dialog open={resetOpen} onOpenChange={setResetOpen}>
        <DialogContent><DialogHeader><DialogTitle>새 테스트 경기를 개설할까요?</DialogTitle></DialogHeader>
          <p className="text-sm text-slate-600">{shared ? "두 화면의 연습 기록을 지우고 0:0부터 다시 시작합니다." : "현재 연습 기록을 지우고 0:0부터 다시 시작합니다."}</p>
